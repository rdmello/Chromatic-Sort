...
BoxCoordinate::BoxCoordinate(int x, int y, int width, int height) 
        :Coordinate(x, y), width(width), height(height) {};

BoundedCoordinate::BoundedCoordinate(int x, int y, int width, int height, int max_x, int max_y)
        :BoxCoordinate(x, y, width, height), max_x(max_x), max_y(max_y) {
    clampToMax(max_x, max_y);
}

void BoundedCoordinate::clampToMax(int max_x, int max_y) {
    this->max_x = max_x;
    this->max_y = max_y;
